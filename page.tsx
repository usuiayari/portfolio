...
import { Inter, Noto_Sans_JP } from 'next/font/google'; // next/fontからフォントをインポート
...
const ProfileHeader = ({ presence }: { presence: LanyardData | null }) => {
  ...
  const profile = {
    ...
    avatarUrl: "/avatar.png", // publicフォルダに画像を配置した場合
  ...
  return (
    <header className="flex flex-col md:flex-row items-center space-y-6 md:space-y-0 md:space-x-10">
      <div className="animate-on-scroll" style={{ transitionDelay: '100ms' }}>
        <Image
          src="https://usuiayari.rumy.jp/avatar.png"
          alt="プロフィール画像"
          width={128}
          height={128}
          className="rounded-full ring-4 ring-gray-700"
          unoptimized={true} // 外部URLの画像を最適化なしで使用
        />
      </div>
        ...
        >
            <div className="flex items-center">
                <div className="relative">
                    <Image src={avatarUrl} alt={discord_user.username} className="w-12 h-12 rounded-full" width={48} height={48} unoptimized={true} />
    ...
    <section className="mt-16 md:mt-24">
      ...
      <div className="mt-8 grid grid-cols-2 md:grid-cols-6 gap-6">
        {minecraftSkills.map((skill, index) => (
          ...
          >
            <Image src={skill.iconUrl} alt={`${skill.name} icon`} className="w-10 h-10" width={40} height={40} unoptimized={true} />
...
const Footer = () => {
    return (
        <footer className="text-center py-8">
            <p className="text-gray-500">&copy; {new Date().getFullYear()} Ayari Usui. All Rights Reserved.</p>
        </footer>
    );
}
...
export default function PortfolioPage() {
    ...
    const GITHUB_USERNAME: string = 'usuiayari'; // 型をstringと明示
    
    // ▼▼▼ あなたのDiscordユーザーIDに書き換えてください ▼▼▼
    const DISCORD_USER_ID: string = '1168902836478804030'; // 型をstringと明示
    ...
    useEffect(() => {
      const fetchGitHubProjects = async () => {
        try {
          ...
          setRepos(data);
        } catch (err: unknown) { // anyの代わりにunknownを使い、型安全にエラーを処理
          if (err instanceof Error) {
            setGithubError(err.message);
          } else {
            setGithubError('予期せぬエラーが発生しました');
          }
        } finally {
    ...
    return (
    <div className={`bg-gray-900 text-white min-h-screen ${inter.variable} ${notoSansJp.variable}`}>
       <style>{`
        body {
